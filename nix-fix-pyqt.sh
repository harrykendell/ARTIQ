...
if [ ${_} != ${0} ]; then
    export QT_PLUGIN_PATH=$(find /nix/store -maxdepth 4 -mindepth 4 -wholename '/*/lib/qt-5.15.12/plugins' | paste -sd ":" -)
unset XDG_SESSION_TYPE
export LANG=C
else
    RED='\033[0;31m'
    NC='\033[0m' # No Color
    echo -e "This script must be run with the dot space script syntax, as in \n${RED}. nix-fix-pyqt.sh${NC}\notherwise it cannot export environment variables into your shell."
fi
