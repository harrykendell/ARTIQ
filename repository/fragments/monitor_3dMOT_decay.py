...
class MOTPhotodiodeMeasurement(Fragment):
    ...
    def unload_3dMOT(self):  # type: ignore
        """
        Unload the 3D MOT by closing the shutter and waiting for the clearout time.

        This closes the shutter now and delays for the unload time.
        """
        self.shutter2d.off()
...
class MeasureMOTWithPDFrag(ExpFragment):
    """
    Plot the loading rate of the 3dMOT by measuring the photodiode signal.
    """
    ...
    def build_fragment(self):
        ...
        self.setattr_param(
            name="MOT3d_unload_time",
            param_class=FloatParam,
            description="duration of 3d mot switch off",
            default=100 * s,
            unit="s",
            min=1 * s,
            step=0.001,
        )
        self.MOT3d_unload_time: FloatParamHandle
