...
async def test_channel(booster: BoosterAPI, channel, prefix, broker):
    ...
    await booster.settings_interface.set(
        f"channel/{channel}/output_interlock_threshold", att
    )
    ...
    async with channel_on(booster, channel):
        ...
        await booster.settings_interface.set(
            f"channel/{channel}/output_interlock_threshold", -5.6
        )
