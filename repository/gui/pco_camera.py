...
def open_camera(camera_name: str | None = None, camera_serial: int | None = None):
    kwargs = {}
    if camera_name is not None:
        kwargs["name"] = camera_name
    if camera_serial is not None:
        kwargs["serial"] = camera_serial
    return pco.Camera(**kwargs) if kwargs else pco.Camera()
...
def main_gui(camera_name: str | None = None, camera_serial: int | None = None):
    app = QApplication([])
    # don't specify an interface or unclosed cameras cause indefinite hangs
    try:
        with open_camera(camera_name=camera_name, camera_serial=camera_serial) as cam:
            init_cam(cam)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PCO camera GUI")
    parser.add_argument(
        "--serial",
        type=int,
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Open a specific camera by SDK camera name.",
    )
    args = parser.parse_args()
    main_gui(camera_name=args.name, camera_serial=args.serial)
