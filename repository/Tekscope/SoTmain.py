...
def main():
    ...
    while True:
        try:
            find_list, return_counter, instrument_description = vpp43.find_resources(
    ...
    resource_ids = []
    for a, item in enumerate(resource_names):
        interface_type, _ = vpp43.parse_resource(
            resource_manager.session, resource_names[a]
        )
        if interface_type == VI_INTF_ASRL:
            ...
            while True:
                try:
                    ...
                    resource_ids.append(
                        scopename[0] + "-" + scopename[1] + " " + scopename[2]
                    )
                    break
                except VisaIOError:
    ...
    while True:
        try:
            ...
            print("Invalid Input, Please Try Again")
            # validate duration_time and echo to user
            if duration_time <= 0:
                print("Invalid duration: must be > 0 minutes. Exiting.")
                raise SystemExit()
            print("Capture duration set to " + str(duration_time) + " minutes.")
    # Query from user if a trigger should be forced to execute periodically
    print("If no trigger is received, would you like to")
    Forcetrig = str(raw_input(" periodically force a trigger? (Y/N) \n"))
    if ("Y" in Forcetrig) or ("y" in Forcetrig):
        Forcetrig = True
        while True:
            try:
                Forcetrig_time = float(
                    raw_input(
                        "How many seconds should I "
    ...
    if "TDS" in scopename[1] and "20" in scopename[1]:
        ...
        HPos = float(
            (Hscale * 5 + float(scope.ask("HORIZONTAL:POSITION?")))
            / (Hscale * 10)
            * 100
        )
        ...
        HDelay = float(scope.ask("HORizontal:DELay:TIME?"))
        HPos = float(
            (Hscale * 5 + float(scope.ask("HORIZONTAL:DELay:TIME?")))
            / (Hscale * 10)
            * 100
        )
    ...
    scope.write("data:start 1;stop " + recLen + ";:data:encdg rpbinary;:DESE 1;:*ESE 1")
    scope.write("wfmoutpre:bit_nr 8")
    ...
    print("Starting Captures on " + ctime() + "\n")
    ...
    while time() < end_time:
        ...
        while waiting == 1:
            ...
            if looptime > end_time:
                if scope.ask("TRIGGER:STATE?") == "READY":
                    print("No trigger event for final capture")
            ...
            if Forcetrig:
                if (looptime + Forcetrig_time) < time():
                    ...
                    capturelog.append(
                        "Trigger forced at " + str(ctime(triggertime)) + "\n"
                    )
        ...
        if (logmode == 1 or logmode == 2) and scope.ask("TRIGGER:STATE?") != "READY":
                    ...
                    if LockFP == False:
                        ...
                        if "TDS" in scopename[1] and "20" in scopename[1]:
                            ...
                            HPos = float(
                                (
                                    Hscale * 5
                                    + float(scope.ask("HORIZONTAL:DELay" + ":TIME?"))
                                )
                                / (Hscale * 10)
                                * 100
                            )
        ...
        if (logmode == 0 or logmode == 2) and scope.ask("TRIGGER:STATE?") != "READY":
            ...
            while True:
                try:
                    ...
                    print(
                        "***Hard copy did not terminate correctly for "
                        + str(filenumber)
                        + "***"
                    )
                    print("    However the image may still be OK")
                    capturelog.append(
                        "***Hard copy did not terminate correctly"
                        " for " + str(filenumber) + "*** \n"
                    )
    ...
    print("Completed captures on " + ctime() + "\n")
    if LockFP:
