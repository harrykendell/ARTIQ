...
class SinaraTester(EnvExperiment):
    ...
    def test_urukuls(self):
        ...
        for channel_name, channel_dev in self.urukuls:
            if not isinstance(channel_dev, AD9910) or not isinstance(
                channel_dev.sync_data, SyncDataEeprom
            ):
                ...
                print(
                    "{}\t{} {}".format(channel_name, sync_delay_seed, io_update_delay)
                )
    ...
    def test_samplers(self):
        ...
        for card_name, card_dev in self.samplers:
            ...
            for channel in range(8):
                print(
                    "Apply 1.5V to channel {}. Press ENTER when done.".format(channel)
                )
                input()
    ...
    def set_phaser_frequencies(self, phaser, duc, osc):
        ...
        for i in range(len(osc)):
            ...
            phaser.channel[0].oscillator[i].set_amplitude_phase(0.2)
            phaser.channel[1].oscillator[i].set_frequency(-osc[i])
            phaser.channel[1].oscillator[i].set_amplitude_phase(0.2)
            delay(1 * ms)
    ...
    def test_grabbers(self):
        ...
        print(
            "Activate the camera's frame grabber output, type 'g', press "
