...
class KDC101:
    ...
    def plot_trapezoid_profile_v_vs_t(
        ...
        print(
            f"Total time (t3): {solve_t3(max_velocity, deceleration, self.s4, self.s3, t2)} s"
        )
    ...
    def set_bow_index(self, sleep_time, index):
        """
        Set the bow index. value from 0 to 18, it is used for s profile of the motion. it actually represent jerk, derivateve of acceleration, higer value means it spends more time in acceleration and deceleration phase making the motion smoother.
        """
        command = bytearray(
            [
                0xF4,
        ...
        index_bytes = index.to_bytes(2, byteorder="little", signed=False)
        command.extend(index_bytes)
        self.ser.write(command)
        time.sleep(sleep_time)
