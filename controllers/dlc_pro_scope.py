...
def main():
    try:
        with DLCpro(NetworkConnection("192.168.0.4")) as dlcpro:
            ...
            ch1_available = (
                dlcpro.laser2.scope.channel1.signal.get() != -3
            )  # Signal is 'none'
            ch2_available = dlcpro.laser2.scope.channel2.signal.get() != -3

            # Set label and unit of X axis
            laxis.set_xlabel(
                "{} [{}]".format(
                    dlcpro.laser2.scope.channelx.name.get(),
                    dlcpro.laser2.scope.channelx.unit.get(),
                )
            )
            ...
            if ch1_available:
                ...
                red.set_ylabel(
                    "{} [{}]".format(
                        dlcpro.laser2.scope.channel1.name.get(),
                        dlcpro.laser2.scope.channel1.unit.get(),
                    ),
                    color="red",
                )
                ...
                if (
                    "l" in lock_candidates.keys() and lock_state == 3
                ):  # State is 'Selected'
                    red.plot(
                        lock_candidates["l"]["x"],
                        lock_candidates["l"]["y"],
                ...
                if lock_state == 5:  # State is 'Locked'
                    background_trace = extract_float_arrays(
                        "xy", dlcpro.laser2.dl.lock.background_trace.get()
                    )
            ...
            if ch2_available:
                ...
                blue.set_ylabel(
                    "{} [{}]".format(
                        dlcpro.laser2.scope.channel2.name.get(),
                        dlcpro.laser2.scope.channel2.unit.get(),
                    ),
                    color="blue",
                )
            ...
            pyplot.tight_layout()
            pyplot.subplots_adjust(left=0.15, right=0.85)
            pyplot.show()
